...
void *send_to_slave(void *arg) {
    ...
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    CPU_SET(slave % sysconf(_SC_NPROCESSORS_ONLN), &cpuset); // Bind to a specific core
    if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset) != 0) {
        perror("Failed to set CPU affinity");
    } else {
        printf("Thread for slave %d bound to core %ld\n", slave, slave % sysconf(_SC_NPROCESSORS_ONLN));
    }
    ...
    for (int i = 0; i < rows_for_this_slave; i += CHUNK_SIZE) {
        ...
        // Simulate bandwidth (e.g., 12 Mbps)
        // double delay_in_seconds = (double)total_bytes / 1500000.0; // Adjust for 12 Mbps
